...
def load_calibration_data(calib_path: str):
...
def combine_concatenated_EMPAD2_AB_big(max_data_load, rawfile, g1A, g1B, g2A, g2B, offA, offB):
    ...
    cdef int max_number_of_process = mp.cpu_count()
...
def debounce_f(npMat, wide: int, w2: int):
    cdef double range1 = -200 - (wide / 2)
    cdef double range2 = 220 - (wide / 2)
    cdef double sum_y, sum_xy, sum_x2y, sum_x2, sum_x4, aVal, bVal, comx
...
def combine_from_concat_EMPAD2_AB_big(max_data_load: int, raw_file, backgd_file, g1A, g1B,
                                      g2A, g2B, offA, offB, flatfA, flatfB):
    npFramesBack, nFramesBack = combine_concatenated_EMPAD2_AB_big(max_data_load, backgd_file, g1A,
                                                                   g1B, g2A, g2B, offA, offB)
    ...
    npFrames, nFrames = combine_concatenated_EMPAD2_AB_big(max_data_load, raw_file, g1A, g1B, g2A,
                                                           g2B, offA, offB)
...
def generate_out_put(out_put, out_put_path, out_put_file_name):
    ...
    with open(file_path, "wb") as file:
        ...
        file.flush()
        file.close()
        print(out_put_file_name + ' was generated into ' + out_put_path + ' successfully.')
...
def run_empadx(config_path: str):
